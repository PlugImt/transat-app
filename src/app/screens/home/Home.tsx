...
import { WeatherSkeleton, WeatherWidget, WidgetList } from "./components";
...
const { data: user } = useUser();
  ...
  const { items: draggableWidgets, reorder } = useWidgetOrder(
    enabledWidgets,
    updateOrder,
    getWidgetComponent
  );
...
const handleCustomizationSave = async (updated: WidgetPreference[]) => {
    await updateOrder(updated);
    setShowCustomizationModal(false);
  };

  if (widgetsLoading) return <HomeLoading />;
