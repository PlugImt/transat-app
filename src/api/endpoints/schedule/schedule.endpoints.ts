...
import type { CalendarData, UpdateUserSchedule, UserSchedule } from "@/dto";
...
export const getInteSchedule = async (): Promise<CalendarData | null> => {
  const api = await getApiInstance();

  try {
    const response = await api.request<CalendarData>({
      url: API_ROUTES.schedule_inte,
      method: Method.GET,
    });
