...
import {
  useAssociationReservations,
  useReservationDisplayData,
} from "@/hooks/services/reservation";
import type { AppNavigation } from "@/types";

export const AssociationReservationWidget = ({ cssociationId }: { cssociationId: number }) => {
  const { t } = useTranslation();
  const navigation = useNavigation<AppNavigation>();
  const { data, isPending, isError } = useAssociationReservations(cssociationId);
  const displayData = useReservationDisplayData(data);

  if (isPending) {
    return <AssociationReservationWidgetSkeleton />;
  }
...
export const AssociationReservationWidgetSkeleton = () => {
  const { t } = useTranslation();
  return (
