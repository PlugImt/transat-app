...
export const WashingMachine: FC = () => {
    ...
    const { data, isPending, isFetching, isError, error, refetch } = useWashingMachines();
    ...
    const washingMachines = data?.filter((machine) => machine.nom_type.trim() === 'LAVE LINGE');
    const dryers = data?.filter((machine) => machine.nom_type.trim() !== 'LAVE LINGE');

    return (
        <Page onRefresh={refetch} refreshing={isFetching} className="flex-col gap-6">
