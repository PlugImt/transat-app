...
export const Timetable = () => {
  ...
  const { data: edt, refetch, isPending: isPendingEdt, isError, error } = useTimetableForWeek(
      user?.email || '',
      selectedDate,
  );
  ...
  const filteredCourses = parsedEdt?.filter((course : Course) =>
        ...
        <View className="gap-2">
          {(edt === null || isError) && (
              <View>
                ...
                {error && <Text style={{ color: theme.destructive }} className="italic">{error.message}</Text>}
              </View>
          )}
            ...
            <Animated.View className="flex-row h-full" style={animatedStyle}>
              ...
              <View className="flex-1 relative pt-4">
                ...
                {filteredCourses?.map((cours: Course, i: number) => {
                  ...
                  return (
                      <View key={i} style={{ top, height }} className="absolute left-0 right-0 px-2">
