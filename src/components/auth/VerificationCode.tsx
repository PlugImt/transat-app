...
export const VerificationCodeModal: React.FC<VerificationCodeModalProps> = ({
    ...
    const handleVerify = async () => {
        ...
        try {
            const response = await axios.post('https://transat.destimt.fr/api/auth/verify-account', {
                email,
    ...
    const requestNewCode = async () => {
        ...
        try {
            await axios.post('https://transat.destimt.fr/api/auth/verification-code', { email });
                    ...
                    <View className="w-full items-center">
                        ...
                        >
                            {isResending ? (
                                ...
                                <Text className="text-l text-[#ec7f32] mt-4">{t('auth.resendCode')}</Text>
                            )}
                        </TouchableOpacity>

                        <View className="flex w-full flex-row justify-between">
                            <TouchableOpacity onPress={onClose} className="p-2">
                                <Text className="text-base text-[#ec7f32]">{t('common.cancel')}</Text>
                            </TouchableOpacity>
                            ...
                            >
                                {isLoading ? (
                                    ...
                                    <Text className="text-base text-white">{t('common.verify')}</Text>
                                )}
                            </TouchableOpacity>
                        </View>
                    </View>
